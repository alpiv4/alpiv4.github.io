...
Why would we use a callable? But sometimes we don't know which function to call until later. An example could be a button, that does different things depending on the situation.
...
With a callable, we just store which function to call ahead of time.
...
A special case for that are signals. We already used callables in them.
...
As you know you connect to functions in the ready method, which is only run once when the node is ready. But how is it able to listen for signals when we do not check every frame, why don't we put the connection in the process function? Because we **store** it with the callable. 
